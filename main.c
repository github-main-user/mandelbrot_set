...
static const char* GRADIENT = ".`,:;i1|tLs_-+^~ \"I<>l?7jvcft/\\()=~#<>Xz{[]}$bmwqpdbkhao*#MW&8%B@$";
const uint8_t GRAD_LENGTH = get_str_lenght(GRADIENT);
const uint32_t MAX_ITERS = 256;

uint8_t get_str_lenght(char* str)
{
	uint8_t i = 0;
	while (*str != '\0')
	{
...
char** calculate_set(width, height)
{
	char** set = [width][height];

	for (uint16_t y = 0; y < height; y++)
	{
		img = convert_number(y, height);
		for (uint16_t x = 0; x < width; x++)
		{
			real = convert_number(x, width);
			...
			set[x][y] = it / (MAX_ITERS / GRAD_LENGTH);
		}
	}
	return set;	
}
